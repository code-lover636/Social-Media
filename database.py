...
def create_db():
    mydb = mysql.connector.connect(host='localhost', user='root', passwd='Socket@11')
...
def get_data():
    mydb = mysql.connector.connect(host='localhost', user='root', passwd='Socket@11', database='LIBRARY_RECORDS')
    my_cursor = mydb.cursor()
...
def add_data(bookid,bookname,author):
    ...
    my_cursor.execute("INSERT INTO books VALUES(%s, %s, %s, %s)", (bookid,bookname,author,'available'))
    mydb.commit()
